...
- !BODY
  ...
  - !Step
    ...
    assert meta_node._value == {'domain': 'sales/cash', 'name': 'make_cash_sale', 'version': '1.0', 'status': 'testing'}

    body_node = result[TagName.BODY]
    assert body_node.tag == TagName.BODY
    first_step_node = body_node._value[0]
    assert first_step_node.tag == TagName.Step
    assert first_step_node.name == 'calculate_total'
    action_node = first_step_node._value['body'][0]
    assert action_node.tag == TagName.Action
    assert isinstance(action_node, ActionTag)
    assert action_node.identity == 'specifications/testing::calculate_total==1.0'

    second_step_node = body_node._value[1]
    assert second_step_node.tag == TagName.Step
    assert second_step_node.name == 'create_sale'
    first_action_node = second_step_node._value['body'][0]
    assert first_action_node.tag == TagName.Action
    assert isinstance(first_action_node, ActionTag)
    assert first_action_node.identity == 'specifications/testing::subtract==1.0'
    second_action_node = second_step_node._value['body'][1]
    assert second_action_node.tag == TagName.Action
    assert isinstance(second_action_node, ActionTag)
    assert second_action_node.identity == 'specifications/testing::create_sale==1.0'
